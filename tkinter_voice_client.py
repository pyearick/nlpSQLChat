# Enhanced tkinter_voice_client.py with interruptible TTS

# tkinter_voice_client.py imports
import os
import sys
import json
import requests
import threading
import time
import webbrowser  # NEW - for opening downloaded files
import re  # NEW - for filename extraction
...
class VoiceClientGUI:
    def __init__(self, root):
        ...
        self.stop_speech_requested = False  # NEW: Flag to stop speech
    ...
    def create_enhanced_input_area(self, parent):
        ...
        # STOP button - prominently placed and always visible
        self.stop_btn = ttk.Button(voice_frame, text="⏹️ STOP", command=self.stop_all_speech,
    ...
    def speak_text(self, text):
        """Enhanced speak text with interruption support"""
        ...
        def speak_with_interruption():
            try:
                ...
                self.root.after(0, lambda: self.voice_status.config(text="Speaking... (click STOP to interrupt)"))
                ...
                for i, sentence in enumerate(sentences):
                    ...
                    if self.stop_speech_requested:
                        self.root.after(0, lambda: self.voice_status.config(text="Speech interrupted"))
                        break

                    # Update status with progress
                    progress = f"Speaking... ({i + 1}/{len(sentences)}) - STOP to interrupt"
                    ...
                    try:
                        ...
                        # Brief pause between sentences (and chance to check for stop)
                        if not self.stop_speech_requested and i < len(sentences) - 1:
                            time.sleep(0.2)
                ...
                self.root.after(0, lambda: self.stop_btn.config(state=tk.NORMAL))  # Keep stop button enabled
    ...
    def split_into_sentences(self, text):
        """Split text into sentences for interruptible speech"""
        # Simple sentence splitting - you could make this more sophisticated
        import re
        ...
        return final_sentences if final_sentences else [text]  # Fallback to original text
    ...
    def stop_all_speech(self):
        ...
        self.log_message("🛑 All speech operations stopped", "system")

    def speak_last_response(self):
        """Speak the last assistant response"""
        if hasattr(self, 'last_response'):
            if self.is_speaking:
                self.log_message("Speech already in progress. Click STOP to interrupt first.", "system")
            else:
                self.speak_text(self.last_response)
        else:
            self.log_message("No response to speak", "system")

    # Updated tkinter client export handling
    ...
    def is_export_response(self, response):
        ...
        export_indicators = [
            "Exported" and "rows to" in response,
    ...
    # Also update your send_message method to use the new handler
    ...
    def query_server_enhanced(self, question, export_format=None):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "Exported" in answer and "rows to:" in answer:
    ...
    def handle_export_response(self, response):
        ...
        if "rows to:" in response:
            try:
                filename = response.split("rows to: ")[1].strip()
                filename = os.path.basename(filename)  # Get just the filename

                # Show download option
                download_msg = f"\n🔽 File ready for download: {filename}"
                self.log_message(download_msg, "system")
    ...
    # ... [Include other existing methods like create_toolbar, create_chat_area,
    #      create_status_bar, log_message, test_connection, show_downloads,
    #      show_table_sizes, etc. - they remain the same]
    ...
    # Include placeholder methods for completeness - implement as needed
    ...
    def show_sizes_window(self, sizes):
        ...
        ttk.Button(button_frame, text="Refresh", command=lambda: [size_window.destroy(), self.show_table_sizes()]).pack(
...
def main():
    ...
    app.log_message("🛑 Use the STOP button to interrupt speech at any time", "system")
