...
class DatabasePlugin:
    ...
    @kernel_function(
        ...
        description=(
                ...
                "- Product mapping: SELECT [Product] FROM rightStock_ProductOEs WHERE [OE] = 'PFF5225R' "
                "- MSRP from OEPriceBookPBI: SELECT [Part Number], [Dealer List Price], [Supperseded Flag] FROM OEPriceBookPBI WHERE [Part Number] = 'PFF5225R' "
                "- Competition from InternetCompData: SELECT [Competitor Name], [Price], [Availability] FROM InternetCompData WHERE [OEAN] = 'PFF5225R' "
                "- Supplier options from Suppliers: SELECT [Name], [collection] FROM Suppliers WHERE [OEAN] = 'PFF5225R' "
                "- eBay activity from ebayWT: SELECT COUNT(*) as ListingCount, AVG(Price) as AvgEbayPrice FROM ebayWT WHERE OEAN = 'PFF5225R' "
                "- Our sales performance: SELECT p.Product, SUM(s.Sales) as TotalSales, SUM(s.Quantity) as TotalQty FROM rightStock_ProductOEs p JOIN pmsalespbi s ON p.[Product] = s.Product WHERE p.[OE] = 'PFF5225R' GROUP BY p.Product "
                "- Performance scoring: SELECT p.Product, r.OverallScore, r.StockScore, r.CompScore FROM rightStock_ProductOEs p JOIN rightScore_results r ON p.[Product] = r.Product WHERE p.[OE] = 'PFF5225R' "
                ...
                "- Product OEAN mapping: SELECT [Product], [OE] FROM rightStock_ProductOEs WHERE [Product] = 'CHR0406R' "
                ...
                "- eBay activity for product OEANs: SELECT p.[Product], p.[OE], COUNT(e.OEAN) as ListingCount, AVG(e.Price) as AvgPrice FROM rightStock_ProductOEs p LEFT JOIN ebayWT e ON p.[OE] = e.OEAN WHERE p.[Product] = 'CHR0406R' GROUP BY p.[Product], p.[OE] "
                "- Product sales performance: SELECT Product, SUM(Sales) as TotalSales, SUM(Quantity) as TotalQty FROM pmsalespbi WHERE Product = 'CHR0406R' GROUP BY Product "
                "- Product performance scores: SELECT Product, OverallScore, StockScore, CompScore FROM rightScore_results WHERE Product = 'CHR0406R' "
                ...
                "Each table uses different field names for part identification - USE THE EXACT FIELD NAME FOR EACH TABLE: "
                "- rightStock_ProductOEs: [OE] (maps internal products to OEANs) "
                "- OEPriceBookPBI: [Part Number] (OEAN field with brackets) "
                "- InternetCompData: [OEAN] (OEAN field with brackets) "
                "- Suppliers: [OEAN] (OEAN field with brackets) "
                "- ebayWT: OEAN (OEAN field without brackets) "
                "- ebayWT_NF: OEAN (OEAN field without brackets) "
                "- ebayNF_SupplierMatch: OEAN (OEAN field without brackets) "
                "- pmsalespbi: Product (internal product code) "
                "- rightScore_results: Product (internal product code) "

                "CRITICAL JOIN PATTERNS: "
                "- To get OEANs for a product: rightStock_ProductOEs.[OE] WHERE [Product] = 'CHR0406R' "
                "- To get product for an OEAN: rightStock_ProductOEs.[Product] WHERE [OE] = '17127531579' "
                "- MSRP by OEAN: OEPriceBookPBI.[Part Number] = '17127531579' "
                "- Competition by OEAN: InternetCompData.[OEAN] = '17127531579' "
                "- Suppliers by OEAN: Suppliers.[OEAN] = '17127531579' "
                "- eBay by OEAN: ebayWT.OEAN = '17127531579' (NO BRACKETS) "
                "- Sales by product: pmsalespbi.Product = 'CHR0406R' "
                "- Scoring by product: rightScore_results.Product = 'CHR0406R' "
                ...
                "5. eBay Market: SELECT COUNT(*) as Listings, AVG(Price) as AvgPrice FROM ebayWT WHERE OEAN = '17127531579' "
                ...
                "- Price comparison across sources: SELECT 'MSRP' as Source, [Dealer List Price] as Price FROM OEPriceBookPBI WHERE [Part Number] = 'PFF5225R' UNION ALL SELECT 'Competition', AVG([Price]) FROM InternetCompData WHERE [OEAN] = 'PFF5225R' UNION ALL SELECT 'eBay', AVG(Price) FROM ebayWT WHERE OEAN = 'PFF5225R' "
